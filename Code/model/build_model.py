...
def build_maskrcnn(configs):
  ...
  if pretrained_model_path != "" and public_model:
    ...
    old_pretrained_dict = torch.load(pretrained_model_path, map_location=torch.device('cpu') )
    pretrained_dict = OrderedDict()
    for k, v in old_pretrained_dict.items():
      if 'module' in k:
        pretrained_dict[''.join(k.split('module.')[1])] = v
      else:
        pretrained_dict[k] = v
...
def build_airobj(configs):
  ...
  if pretrained_model_path != "":
    model_dict = model.state_dict()
    # pretrained_dict = torch.load(pretrained_model_path, map_location=torch.device('cuda:{}'.format('0')))
    old_pretrained_dict = torch.load(pretrained_model_path, map_location=torch.device('cpu') )
    pretrained_dict = OrderedDict()
    for k, v in old_pretrained_dict.items():
      if 'module' in k:
