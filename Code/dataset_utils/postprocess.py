...
def postprocess_detections(new_image_sizes, original_image_sizes, 
                           mask_threshhold=0.15, conf_threshold=0.5, detections=None):
  
  '''
  inputs:
    new_image_sizes: tensor, N*2, [h, w]
    original_image_sizes: tensor, N*2, [h, w]
    mask_threshhold: scalar, optional
    conf_threshold: scalar, optional
    detections: List[Dict[Tensor]], optional
  ...
  if detections is not None:
    for i in range(len(detections)):
      ...
      good_scores = torch.nonzero(sort_scores>conf_threshold).squeeze(1)
      good_inx = idx[good_scores]

      boxes = boxes[good_inx]
      labels = labels[good_inx]
      scores = scores[good_inx]
      masks = (masks[good_inx] > mask_threshhold).float()
